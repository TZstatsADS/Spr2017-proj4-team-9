...
AGupta<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/A Gupta.csv")

AGupta$author.ID<-as.character(AGupta$author.ID)
AGupta$paper.ID<-as.character(AGupta$paper.ID)
AGupta$paper.title<-as.character(AGupta$paper.title)
AGupta$coauthor.names<-as.character(AGupta$coauthor.names)
it_train <- itoken(AGupta$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(AGupta$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(AGupta$author.ID)))
for (i in 1:length(unique(AGupta$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(AGupta), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the AGupta. 
  h_result<-cutree(h,k=length(unique(AGupta$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(AGupta$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(AGupta$author.ID))){
  ...
  author.clust <- vector(length=length(unique(AGupta$author.ID)))
  for (i in 1:length(unique(AGupta$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(AGupta$author.ID)){
...
matching_matrix_hclust <- matching_matrix(AGupta$author.ID,T_hat_overall) 
performance_hclust.AG <- performance_statistics(matching_matrix_hclust)

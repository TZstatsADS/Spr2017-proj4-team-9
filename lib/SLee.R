...
SLee<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/SLee.csv")

SLee$author.ID<-as.character(SLee$author.ID)
SLee$paper.ID<-as.character(SLee$paper.ID)
SLee$paper.title<-as.character(SLee$paper.title)
SLee$coauthor.names<-as.character(SLee$coauthor.names)
it_train <- itoken(SLee$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(SLee$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(SLee$author.ID)))
for (i in 1:length(unique(SLee$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(SLee), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the SLee. 
  h_result<-cutree(h,k=length(unique(SLee$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(SLee$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(SLee$author.ID))){
  ...
  author.clust <- vector(length=length(unique(SLee$author.ID)))
  for (i in 1:length(unique(SLee$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(SLee$author.ID)){
...
matching_matrix_hclust <- matching_matrix(SLee$author.ID,T_hat_overall) 
performance_hclust.SL <- performance_statistics(matching_matrix_hclust)

performance_hclust.SL
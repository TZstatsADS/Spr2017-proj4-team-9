...
JSmith<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/J Smith.csv")

JSmith$author.ID<-as.character(JSmith$author.ID)
JSmith$paper.ID<-as.character(JSmith$paper.ID)
JSmith$paper.title<-as.character(JSmith$paper.title)
JSmith$coauthor.names<-as.character(JSmith$coauthor.names)
it_train <- itoken(JSmith$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(JSmith$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(JSmith$author.ID)))
for (i in 1:length(unique(JSmith$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(JSmith), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the JSmith. 
  h_result<-cutree(h,k=length(unique(JSmith$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(JSmith$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(JSmith$author.ID))){
  ...
  author.clust <- vector(length=length(unique(JSmith$author.ID)))
  for (i in 1:length(unique(JSmith$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(JSmith$author.ID)){
...
matching_matrix_hclust <- matching_matrix(JSmith$author.ID,T_hat_overall) 
performance_hclust.JS <- performance_statistics(matching_matrix_hclust)

...
JMartin<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/J Martin.csv")

JMartin$author.ID<-as.character(JMartin$author.ID)
JMartin$paper.ID<-as.character(JMartin$paper.ID)
JMartin$paper.title<-as.character(JMartin$paper.title)
JMartin$coauthor.names<-as.character(JMartin$coauthor.names)
it_train <- itoken(JMartin$paper.title, 
                   preprocessor = tolower, 
                   tokenizer = word_tokenizer,
                   ids = JMartin$paper.ID,
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(JMartin$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(JMartin$author.ID)))
for (i in 1:length(unique(JMartin$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(JMartin), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the JMartin. 
  h_result<-cutree(h,k=length(unique(JMartin$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(JMartin$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(JMartin$author.ID))){
  ...
  author.clust <- vector(length=length(unique(JMartin$author.ID)))
  for (i in 1:length(unique(JMartin$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(JMartin$author.ID)){
...
matching_matrix_hclust <- matching_matrix(JMartin$author.ID,T_hat_overall) 
performance_hclust.JM <- performance_statistics(matching_matrix_hclust)

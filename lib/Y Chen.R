...
YChen<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/Y Chen.csv")

YChen$author.ID<-as.character(YChen$author.ID)
YChen$paper.ID<-as.character(YChen$paper.ID)
YChen$paper.title<-as.character(YChen$paper.title)
YChen$coauthor.names<-as.character(YChen$coauthor.names)
it_train <- itoken(YChen$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(YChen$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(YChen$author.ID)))
for (i in 1:length(unique(YChen$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(YChen), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the YChen. 
  h_result<-cutree(h,k=length(unique(YChen$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(YChen$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(YChen$author.ID))){
  ...
  author.clust <- vector(length=length(unique(YChen$author.ID)))
  for (i in 1:length(unique(YChen$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(YChen$author.ID)){
...
matching_matrix_hclust <- matching_matrix(YChen$author.ID,T_hat_overall) 

...
JRobinson<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/J Robinson.csv")

JRobinson$author.ID<-as.character(JRobinson$author.ID)
JRobinson$paper.ID<-as.character(JRobinson$paper.ID)
JRobinson$paper.title<-as.character(JRobinson$paper.title)
JRobinson$coauthor.names<-as.character(JRobinson$coauthor.names)
it_train <- itoken(JRobinson$paper.title, 
                   preprocessor = tolower, 
                   tokenizer = word_tokenizer,
                   ids = JRobinson$paper.ID,
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(JRobinson$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(JRobinson$author.ID)))
for (i in 1:length(unique(JRobinson$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(JRobinson), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the JRobinson. 
  h_result<-cutree(h,k=length(unique(JRobinson$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(JRobinson$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(JRobinson$author.ID))){
  ...
  author.clust <- vector(length=length(unique(JRobinson$author.ID)))
  for (i in 1:length(unique(JRobinson$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(JRobinson$author.ID)){
...
matching_matrix_hclust <- matching_matrix(JRobinson$author.ID,T_hat_overall) 
performance_hclust.JR <- performance_statistics(matching_matrix_hclust)

...
MBrown<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/M Brown.csv")

MBrown$author.ID<-as.character(MBrown$author.ID)
MBrown$paper.ID<-as.character(MBrown$paper.ID)
MBrown$paper.title<-as.character(MBrown$paper.title)
MBrown$coauthor.names<-as.character(MBrown$coauthor.names)
it_train <- itoken(MBrown$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(MBrown$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(MBrown$author.ID)))
for (i in 1:length(unique(MBrown$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(MBrown), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the MBrown. 
  h_result<-cutree(h,k=length(unique(MBrown$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(MBrown$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(MBrown$author.ID))){
  ...
  author.clust <- vector(length=length(unique(MBrown$author.ID)))
  for (i in 1:length(unique(MBrown$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(MBrown$author.ID)){
...
matching_matrix_hclust <- matching_matrix(MBrown$author.ID,T_hat_overall) 
performance_hclust.MB <- performance_statistics(matching_matrix_hclust)

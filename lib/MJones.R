...
MJones<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/M Jones.csv")

MJones$author.ID<-as.character(MJones$author.ID)
MJones$paper.ID<-as.character(MJones$paper.ID)
MJones$paper.title<-as.character(MJones$paper.title)
MJones$coauthor.names<-as.character(MJones$coauthor.names)
it_train <- itoken(MJones$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(MJones$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(MJones$author.ID)))
for (i in 1:length(unique(MJones$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(MJones), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the MJones. 
  h_result<-cutree(h,k=length(unique(MJones$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(MJones$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(MJones$author.ID))){
  ...
  author.clust <- vector(length=length(unique(MJones$author.ID)))
  for (i in 1:length(unique(MJones$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(MJones$author.ID)){
...
matching_matrix_hclust <- matching_matrix(MJones$author.ID,T_hat_overall) 
performance_hclust.MJ <- performance_statistics(matching_matrix_hclust)

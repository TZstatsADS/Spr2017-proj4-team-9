...
KTanaka<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/K Tanaka.csv")

KTanaka$author.ID<-as.character(KTanaka$author.ID)
KTanaka$paper.ID<-as.character(KTanaka$paper.ID)
KTanaka$paper.title<-as.character(KTanaka$paper.title)
KTanaka$coauthor.names<-as.character(KTanaka$coauthor.names)
it_train <- itoken(KTanaka$paper.title, 
                   preprocessor = tolower, 
                   tokenizer = word_tokenizer,
                   ids = KTanaka$paper.ID,
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(KTanaka$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(KTanaka$author.ID)))
for (i in 1:length(unique(KTanaka$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(KTanaka), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the KTanaka. 
  h_result<-cutree(h,k=length(unique(KTanaka$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(KTanaka$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(KTanaka$author.ID))){
  ...
  author.clust <- vector(length=length(unique(KTanaka$author.ID)))
  for (i in 1:length(unique(KTanaka$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(KTanaka$author.ID)){
...
matching_matrix_hclust <- matching_matrix(KTanaka$author.ID,T_hat_overall) 
performance_hclust.KT <- performance_statistics(matching_matrix_hclust)

...
CChen<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/C Chen.csv")

CChen$author.ID<-as.character(CChen$author.ID)
CChen$paper.ID<-as.character(CChen$paper.ID)
CChen$paper.title<-as.character(CChen$paper.title)
CChen$coauthor.names<-as.character(CChen$coauthor.names)
it_train <- itoken(CChen$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(CChen$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(CChen$author.ID)))
for (i in 1:length(unique(CChen$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(CChen), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the CChen. 
  h_result<-cutree(h,k=length(unique(CChen$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(CChen$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(CChen$author.ID))){
  ...
  author.clust <- vector(length=length(unique(CChen$author.ID)))
  for (i in 1:length(unique(CChen$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(CChen$author.ID)){
...
matching_matrix_hclust <- matching_matrix(CChen$author.ID,T_hat_overall) 
performance_hclust.CC <- performance_statistics(matching_matrix_hclust)

...
JLee<-read.csv("/Users/xuehan/Desktop/Spr2017-proj4-team-9/data/nameset/J Lee.csv")

JLee$author.ID<-as.character(JLee$author.ID)
JLee$paper.ID<-as.character(JLee$paper.ID)
JLee$paper.title<-as.character(JLee$paper.title)
JLee$coauthor.names<-as.character(JLee$coauthor.names)
it_train <- itoken(JLee$paper.title, 
...
dtm_train_tfidf<-cbind(dtm_train_tfidf,as.numeric(JLee$author.ID))
...
element<-list()
S_star<-vector(length=length(unique(JLee$author.ID)))
for (i in 1:length(unique(JLee$author.ID))){
...
K=14
k=1
lambda1 <- matrix(NA, nrow = nrow(JLee), ncol = K)
S1 <- numeric(K)
acc <- numeric(K)
while (k<(K+1)){
  ...
  #Check the result for the number of cluster equals to the number of unique authors in the JLee. 
  h_result<-cutree(h,k=length(unique(JLee$author.ID)))
  
  #Compute the our own score function S
  S<-vector(length=length(unique(JLee$author.ID)))
  element_s<-list()
  for (i in 1:length(unique(JLee$author.ID))){
  ...
  author.clust <- vector(length=length(unique(JLee$author.ID)))
  for (i in 1:length(unique(JLee$author.ID))){
    author.clust[i]<-as.numeric(names(which.max(table(label[which(h_result==i)]))))
  }
  
  for (i in 1:unique(JLee$author.ID)){
...
matching_matrix_hclust <- matching_matrix(JLee$author.ID,T_hat_overall) 
performance_hclust.JL <- performance_statistics(matching_matrix_hclust)
